...
class googleimagesdownload:
    ...
    # Download Images
    def download_image(self,image_description, image_url,image_format,main_directory,dir_name,count,print_urls,socket_timeout,prefix,print_size,no_numbering,no_download,save_source,img_src,silent_mode,thumbnail_only,format,ignore_urls):
        if not silent_mode:
            if print_urls or no_download:
                print("Image URL: " + image_url)
        if ignore_urls:
        ...
        if no_download:
            return "success","Tanpa Download",None,image_url
        try:
            req = Request(image_url, headers={
                "User-Agent": "Mozilla/5.0 (X11; Linux i686) AppleWebKit/537.17 (KHTML, like Gecko) Chrome/24.0.1312.27 Safari/537.17"})
            try:
                ...
                if no_numbering:
                    # path = main_directory + "/" + dir_name + "/" + prefix + image_description
                    path = main_directory + "/" + dir_name + "/" + prefix + image_name
                else:
                    path = main_directory + "/" + dir_name + "/" + prefix + str(count) + "." + image_name
                    # path = main_directory + "/" + dir_name + "/" + prefix + str(count) + "." + image_description
    ...
    def _get_all_items(self,page,main_directory,dir_name,limit,arguments):
        ...
        while count < limit+1:
            ...
            if object == "no_links":
                ...
                #download the images
                #downloadgambar
                download_status,download_message,return_image_name,absolute_path = self.download_image(object['image_description'],object['image_link'],object['image_format'],main_directory,dir_name,count,arguments['print_urls'],arguments['socket_timeout'],arguments['prefix'],arguments['print_size'],arguments['no_numbering'],arguments['no_download'],arguments['save_source'],object['image_source'],arguments["silent_mode"],arguments["thumbnail_only"],arguments['format'],arguments['ignore_urls'])
                if not arguments["silent_mode"]:
        ...
        for pky in prefix_keywords:                 # 1.for every prefix keywords
            for sky in suffix_keywords:             # 2.for every suffix keywords
                ...
                while i < len(search_keyword):      # 3.for every main keyword
                    iteration = "\n" + "KW No : " + str(i + 1) + " -->" + " KW : " + (pky) + (search_keyword[i]) + (sky)
                    if not arguments["silent_mode"]:
                    ...
                    if arguments['extract_metadata']:
                        ...
                        # export alt txt
                        txt_file = open("alt/"+search_keyword[i]+".txt","w+")
                        for x in range(len(items)):
                            txt_file.write(""+items[x]['image_description']+"\r")
...
def main():
    ...
    for arguments in records:
        ...
        if not arguments["silent_mode"]:
            ...
            print("Total waktu proses : " + str(total_time) + " Seconds")
